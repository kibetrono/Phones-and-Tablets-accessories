                         ...
                         $('#userTable tbody').empty(); // Empty <tbody>
                            ...
                            var total = 0;
                                ...
                                for (var i = 0; i < data2.length; i++) {
                                    //  console.log(data2[i].model_name);

                                    var d= new Date (Date.parse(data2[i].created_at))
                                    var date=d.getFullYear()+'-'+ (d.getMonth() + 1)+'-'+d.getDate()
                                    var product_name = data2[i].model_name;
                                    var sale_price = (data2[i].sale_price).toLocaleString();
                                    var supplier_person = data2[i].supplier_person;
                                    var vender_id = data2[i].vender_id;
                                    let url = "{{ route('supplier.show', ':id') }}";
                                    url = url.replace(':id', vender_id);
                                    var status = data2[i].status;
